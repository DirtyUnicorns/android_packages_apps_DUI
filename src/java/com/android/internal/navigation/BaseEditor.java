...
public abstract class BaseEditor implements Editor {
    ...
    private void toastState(int state) {
        String txt;
        if (state == MODE_ON) {
            txt = DUActionUtils.getString(mContext, "smartbar_editor_toast_starting", DUActionUtils.PACKAGE_SYSTEMUI);
        } else if (state == MODE_OFF) {
            txt = DUActionUtils.getString(mContext, "smartbar_editor_toast_stopping", DUActionUtils.PACKAGE_SYSTEMUI);
        } else {
            txt = DUActionUtils.getString(mContext, "smartbar_editor_toast_unavailable", DUActionUtils.PACKAGE_SYSTEMUI);
        }
